# 使用 Ubuntu 22.04 作为基础镜像，它对现代编译器支持良好
FROM ubuntu:22.04
...
# 1. 安装系统级依赖
# 包括：C++/Fortran 编译器 (GCM 开发必需)、CMake、MPI (并行计算需求)、Git、Wget 和解压工具 [cite: 124, 127]
...
# TorchClim 依赖 LibTorch 来执行 AI 模型的推理 [cite: 134, 146]
...
# 设置 LibTorch 相关的环境变量，以便编译器能找到它 [cite: 146]
...
# 4. 编译 TorchClim 插件 [cite: 130, 149]
